...
class ResNet(nn.Module):
    def __init__(self, block, layers, image_channels, num_classes):
        ...
        self.relu = nn.ReLU()
        self.mapool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        #Resnet layers
        self.layer1 = self._make_layers(CNNBlock, layers[0], out_channel=64, stride=1)
        self.layer2 = self._make_layers(CNNBlock, layers[1], out_channels=128, stride=2)
        self.layer3 = self._make_layers(CNNBlock, layers[2], out_channels=256, stride=2)
        self.layer4 = self._make_layers(CNNBlock, layers[3], out_channels=512, stride=2)
    ...
    def _make_layers(self, block, num_residual, out_channels, stride):
